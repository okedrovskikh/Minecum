...
void Player::processMovement(GLFWwindow*& window, float deltaTime, const World& world)
{
	...
	applyMotion(motion, world.activeChunk);
}

void Player::rayCast(const World& world)
{
	...
	int* order = getOrder();
	for (int l = 0; l < world.activeChunk.size(); l++)
	{
		if (world.activeChunk[l] != nullptr) {
			for (int i = 0; i < CHUNK_SIZE; i++)
			{
				if (world.activeChunk[l]->coordinate[i].type != AIR) {
					world.activeChunk[l]->coordinate[i].chosen = false;

					glm::vec3 coord = world.activeChunk[l]->coordinate[i].coord;
	...
	if (interactionChunkIndex != -1 && interactionBlockIndex != -1)
		world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].chosen = true;
}

void Player::processLeftClick(const World& world)
{
	if (interactionChunkIndex != -1 && interactionBlockIndex != -1) {
		world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].type = AIR;
		world.activeChunk[interactionChunkIndex]->updateMesh();
		if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.x == world.activeChunk[interactionChunkIndex]->bottom.x && world.activeChunk[interactionChunkIndex]->chunks[0] != nullptr)
			world.activeChunk[interactionChunkIndex]->chunks[0]->updateMesh();
		if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.z == world.activeChunk[interactionChunkIndex]->bottom.z && world.activeChunk[interactionChunkIndex]->chunks[1] != nullptr)
			world.activeChunk[interactionChunkIndex]->chunks[1]->updateMesh();
		if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.x == world.activeChunk[interactionChunkIndex]->top.x && world.activeChunk[interactionChunkIndex]->chunks[2] != nullptr)
			world.activeChunk[interactionChunkIndex]->chunks[2]->updateMesh();
		if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.z == world.activeChunk[interactionChunkIndex]->top.z && world.activeChunk[interactionChunkIndex]->chunks[3] != nullptr)
			world.activeChunk[interactionChunkIndex]->chunks[3]->updateMesh();
	}
}

void Player::processRightClick(const World& world)
{
	if (interactionChunkIndex == -1 || interactionBlockIndex == -1)
		return;
	...
	if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].chosen == true) {
		glm::vec3 delta[3] = {
			glm::vec3(sgn(camera.Position.x - world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.x), 0.0f, 0.0f),
			glm::vec3(0.0f, sgn(camera.Position.y - world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.y), 0.0f),
			glm::vec3(0.0f, 0.0f, sgn(camera.Position.z - world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.z))
		};

		int* order = getOrder();
		for (int i = 0; i < 3; i++)
		{
			glm::vec3 newCoord = world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord + delta[i];
			glm::vec3 solution = camera.Position;
	...
	if (done) {
		for (int i = 0; i < world.activeChunk.size(); i++) {
			int index = world.activeChunk[i]->getBlockIndex(newBlock);
			if (index != -1) {
				...
				if (!(collisionX && collisionY && collisionZ)) {
					world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].chosen = false;
					world.activeChunk[i]->coordinate[index].type = chosenBlock;
					world.activeChunk[i]->coordinate[index].chosen = true;
					world.activeChunk[i]->updateMesh();
					if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.x == world.activeChunk[interactionChunkIndex]->bottom.x && world.activeChunk[interactionChunkIndex]->chunks[0] != nullptr)
						world.activeChunk[interactionChunkIndex]->chunks[0]->updateMesh();
					if (world.activeChunk[interactionChunkIndex]->coordinate[interactionBlockIndex].coord.z == world.activeChunk[interactionChunkIndex]->bottom.z && world.activeChunk[interactionChunkIndex]->chunks[1] != nullptr)
						world.activeChunk[interactionChunkIndex]->chunks[1]->updateMesh();
